...
int IsCapture(M0_RADAR_FRAME *radar_data)
{
	...
	if(radar_data->data.L_R ? Radar_PreShoot(pPredictionDataA, radar_data->data) : Radar_PreShoot(pPredictionDataB, radar_data->data))
    {
        radar_data->status = 1;
        return 1;
    }
...
int IsDelete(M0_RADAR_FRAME *radar_data)
{
    ...
    if(radar_data->data.L_R ? PredictionDataA.delete_file_flag : PredictionDataB.delete_file_flag)
    {
        radar_data->status = 2;
        return 1;
    }
...
Radar_Error Radar_GetObjectStatus(Radar_PredictionData_t *pPredictionData, M0_RADAR_DATA_FRAME data)
{
	...
	if (Status == RADAR_ERROR_NONE)
	{
		...
		else if (data.Range_Object1 > 0 && pPredictionData->SpeedData.DeltaV == 0 && pPredictionData->SpeedData.DeltaX == 0)
...
Radar_Error Radar_GetObjectSpeedData(Radar_PredictionData_t *pPredictionData, M0_RADAR_DATA_FRAME data)
{
	...
	if (Status == RADAR_ERROR_NONE)
	{
		if (pPredictionData->SpeedData.InitialDistance != 0)
			pPredictionData->SpeedData.DeltaX = data.Range_Object1 - pPredictionData->SpeedData.InitialDistance;
		else
			pPredictionData->SpeedData.DeltaX = 0;

		pPredictionData->SpeedData.InitialDistance = data.Range_Object1;
...
int IsPreShoot(Radar_PredictionData_t *pPredictionData, M0_RADAR_DATA_FRAME data)
{
	if (pPredictionData->Status == RADAR_PREDICTIONSTATUS_PARKING || pPredictionData->Status == RADAR_PREDICTIONSTATUS_COMING)
		return data.L_R ?  data.Range_Object1 < LEFT_DETECT_DISTANCE : data.Range_Object1 < RIGHT_DETECT_DISTANCE;
	else
		return 0;
}
